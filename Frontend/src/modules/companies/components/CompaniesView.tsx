...
export function CompaniesView() {
  ...
  if (loading) return <GaneshaLoader variant="compact" message="Sincronizando Empresas..." />;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-vintage-600 dark:text-zinc-400 mt-1">Gestión de empresas del sistema</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 stagger-children">
          {companies.map((company) => (
            <VintageCard key={company.id} className="relative">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-semibold text-vintage-800 dark:text-zinc-100 text-sm">{company.name}</h3>
                      {company.parentId && <span className="bg-vintage-100 text-[10px] px-1.5 py-0.5 rounded-full text-vintage-500 dark:text-zinc-400 font-medium">Sucursal</span>}
                    </div>
                    <p className="text-xs text-vintage-500 dark:text-zinc-400 font-mono">{company.taxId}</p>
              ...
              <div className="space-y-2 text-xs text-vintage-600 dark:text-zinc-400 mb-4">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-vintage-100">
                <span className="text-xs font-medium text-vintage-500 dark:text-zinc-400">Moneda: {company.currency}</span>
                <div className="flex gap-1">
                  ...
                  className="p-1.5 rounded-lg hover:bg-vintage-100 text-vintage-500 dark:text-zinc-400 hover:text-vintage-700 transition-colors"
                >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-error/10 text-vintage-500 dark:text-zinc-400 hover:text-error transition-colors"
                >
      ...
      {showForm && (
        <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <motion.div className="relative bg-card rounded-2xl p-6 max-w-lg w-full shadow-xl border border-vintage-200 dark:border-zinc-800" initial={{ scale: 0.95 }} animate={{ scale: 1 }}>
            <h3 className="text-lg font-playfair font-bold text-vintage-800 dark:text-zinc-100 mb-4">{editingCompany ? 'Editar' : 'Nueva'} Empresa</h3>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 text-sm rounded-xl border border-vintage-200 dark:border-zinc-800 text-vintage-700 hover:bg-vintage-50">Cancelar</button>
  ...
  return (
    <div className="relative">
      <select
        ...
        onChange={onChange}
        aria-label={label}
        className="peer w-full px-3 pt-5 pb-2 text-sm bg-card border border-vintage-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-vintage-400 appearance-none"
      >
        {children}
      </select>
      <label className="absolute left-3 top-1.5 text-xs text-vintage-500 dark:text-zinc-400 font-medium pointer-events-none">{label}</label>
