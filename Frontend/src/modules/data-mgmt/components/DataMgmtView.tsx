  ...
  const handleExport = async (title: string, format: 'Excel' | 'PDF') => { 
    ...
    try {
      ...
      if (title.includes('Balanza')) {
        const dummyTotals = { totalDebit: 0, totalCredit: 0, totalBalance: 0 };
        if (format === 'Excel') await exportTrialBalanceExcel([], companyName, 'Período Actual', dummyTotals);
        else await exportTrialBalancePDF([], companyName, 'Período Actual', dummyTotals);
